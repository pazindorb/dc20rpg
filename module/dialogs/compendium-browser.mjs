...
export class CompendiumBrowser extends Dialog {
  ...
  constructor(itemType, lockItemType, dropObject, preSelectedFilters, dialogData = {}, options = {}) {
    ...
    if (itemType === "inventory") {
      this.allItemTypes = DC20RPG.DROPDOWN_DATA.inventoryTypes;
      itemType = "weapon";
    }
      ...
      this.allItemTypes = DC20RPG.DROPDOWN_DATA.allItemTypes;
    }
  ...
  _prepareFilters(preSelectedFilters) {
    ...
    return {
      ...
      feature: {
        ...
        featureType: this._filter("select", parsedFilters["featureType"], DC20RPG.DROPDOWN_DATA.featureSourceTypes)
      },
      technique: {
        techniqueOrigin: this._filter("text", parsedFilters["techniqueOrigin"]),
        techniqueType: this._filter("select", parsedFilters["techniqueType"], DC20RPG.DROPDOWN_DATA.techniqueTypes)
      },
      spell: {
        spellOrigin: this._filter("text", parsedFilters["spellOrigin"]),
        spellType: this._filter("select", parsedFilters["spellType"], DC20RPG.DROPDOWN_DATA.spellTypes),
        magicSchool: this._filter("select", parsedFilters["magicSchool"], DC20RPG.DROPDOWN_DATA.magicSchools),
      ...
      weapon: {
        weaponType: this._filter("select", parsedFilters["weaponType"], DC20RPG.DROPDOWN_DATA.weaponTypes),
        weaponStyle: this._filter("select", parsedFilters["weaponStyle"], DC20RPG.DROPDOWN_DATA.weaponStyles),

      },
      equipment: {
        equipmentType: this._filter("select", parsedFilters["equipmentType"], DC20RPG.DROPDOWN_DATA.equipmentTypes)
      },
      consumables: {
        consumableType: this._filter("select", parsedFilters["consumableType"], DC20RPG.DROPDOWN_DATA.techniqueTypes)
      }
    }
  }
