...
export class AdvancementConfiguration extends Dialog {
  ...
  async _onEditMacro() {
    ...
    const originalMacro = this.item.system.advancements[this.key].macro; 
    const advancement = this.advancement;
    const macro = await createTemporaryMacro(originalMacro, this.item, {itemUuid: this.item.uuid, advancement: advancement, updatePath: `system.advancements.${this.key}.macro`});
  ...
  async _onDrop(event) {
    ...
    if (!["feature", "maneuver", "infusion", "spell", "weapon", "equipment", "spellFocus"].includes(item.type)) return;
